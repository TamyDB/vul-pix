    ...
    <div className="min-h-screen font-['Poppins',sans-serif] bg-gradient-to-b from-[#B5DBFF] to-[#458CBF]">
      ...
      <div className="min-h-screen flex flex-col items-center px-[5%] pb-10 pt-[100px]">
        ...
        <div className="relative flex items-center gap-6 bg-[#DAE6F3] rounded-2xl p-4 w-full max-w-[950px] min-h-[340px] flex-col md:flex-row">
          ...
          {/* Voltar */}
          <span
            className="absolute left-5 top-2 text-[#0179be] text-3xl cursor-pointer transition-transform duration-300 hover:-translate-x-1 hover:scale-125 select-none"
            onClick={() => navigate('/')}
          >
            ‹
          </span>
          ...
          {/* Favorito */}
          <span
            className="absolute right-7 top-4 text-[#0179be] text-3xl cursor-pointer transition-transform duration-300 hover:scale-125 select-none"
          ...
          <img
            src={card?.image}
            alt={card?.name}
            className="w-[180px] rounded-lg flex-shrink-0 mx-auto mt-8 max-w-[55%] md:max-w-[25%] md:ml-14 md:mt-5"
          />
          ...
          <div className="flex flex-col gap-2 flex-1 items-center text-center md:items-start md:text-left">
            ...
            <h3 className="text-[#00196d] font-bold text-2xl md:ml-5">
              R$ {card?.price?.toFixed(2)}
            </h3>
            <p className="text-[#0179be] text-lg md:ml-12">
              Em até 4x R$ {(card?.price / 4)?.toFixed(2)}
            </p>
        ...
        {/* Botões */}
        <div className="flex flex-col sm:flex-row gap-4 pt-8 pb-10 w-full max-w-[950px] justify-center items-center">
          <button
            type="button"
            className="w-[85%] sm:w-[45%] max-w-[225px] h-[50px] rounded-3xl px-6 text-lg border-2 border-[#4a90b8] bg-white text-[#0179be] cursor-pointer transition-all duration-200 hover:opacity-85 hover:scale-103"
          >
            🛒 Carrinho
          </button>
          <button
            type="button"
            className="w-[85%] sm:w-[45%] max-w-[225px] h-[50px] rounded-3xl px-6 text-lg bg-[#4a90b8] text-white border-none cursor-pointer transition-all duration-200 hover:opacity-85 hover:scale-103"
          >
            Compre agora
          </button>
        </div>
