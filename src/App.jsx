...
const PokemonEmbed = ({ isVisible, onOpen, onClose }) => {
  ...
  return (
    <div className="fixed bottom-8 right-8 z-40 w-124 h-80 bg-transparent group">
      <div id="game" className="w-full h-full rounded-lg overflow-hidden"></div>
      <button onClick={onClose} className="absolute -top-8 right-0 text-zinc-500 hover:text-zinc-700 font-bold">
        ✕
      </button>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-6">
      <a href="https://github.com/tamydb/vul-pix" target="_blank">
        <img src={vulpixLogo} className="h-32" alt="Vul-Pix logo" />
      </a>
      <h1 className="text-6xl text-[#213547] font-bold mb-8">Vul-Pix</h1>
      ...
      <p className="font-light">
        Clique aqui para ser redirecionado ao repositório do{' '}
        <a className="font-bold" href="https://github.com/tamydb/vul-pix">Github</a>
      </p>
      <button
        className="cursor-pointer px-4 py-2 bg-white font-medium text-black border border-white hover:border-black transition-colors duration-300 rounded-lg"
